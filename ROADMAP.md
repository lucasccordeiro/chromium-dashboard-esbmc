...
| Tier 3 — self-certify contracts | `is_privacy_eligible`, `is_testing_eligible`, `is_adoption_eligible`, `is_eligible_dispatch` (4 pairs) | All Phase 1 + Phase 2 SUCCESSFUL; buggy variants all FAILED. Dispatch harness required redesign: `nondet_bool()` in list comprehension gives fresh draw per access in ESBMC-Python — fixed by using explicit scalar args. |
| Tier 4 — SLO state machine | `record_vote_index_safety`, `overdue_detection` (2 pairs) | All Phase 1 + Phase 2 SUCCESSFUL; buggy variants all FAILED. |
